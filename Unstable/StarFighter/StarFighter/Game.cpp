...
{
	this->hud.Update(m_armor, m_armor_max, m_shield, m_shield_max, m_money, m_graze_count, m_hazard_level, scene_name, level, level_max, xp, xp_max, this->GetPlayerStatsMultiplierForLevel(level), deltaTime, hub, focused_item_type, f_name, f_max_speed, f_hyperspeed, f_hyperspeed_bonus,
...
void Game::ApplyLevelModifiers()
{
	//applyig on enemies
	for (std::vector<Independant*>::iterator it = this->sceneIndependantsLayered[EnemyObjectLayer].begin(); it != this->sceneIndependantsLayered[EnemyObjectLayer].end(); it++)
	{
		if (*it == NULL)
			continue;
	
		(*it)->ApplyLevelModifiers();
	}
}