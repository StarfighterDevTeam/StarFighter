...
void HelpInterface::Init(string& text_title, string& text_body, sf::Vector2f upper_left_anchor_position)
{
	...
	if (text_title.empty() == false)
	{
		...
		m_text_title.setFillColor((*CurrentGame).m_dico_colors[Color_Green_System]);
	...
	if (text_body.empty() == false)
	{
		...
		m_text_body.setFillColor(sf::Color::White);
	...
	m_panel->m_shape_container.setOrigin(sf::Vector2f(m_panel->m_shape_container.getSize().x * 0.5f, m_panel->m_shape_container.getSize().y * 0.5f));
	m_panel->m_shape_container.setFillColor(sf::Color::Black);
	m_panel->m_shape_container.setPosition(sf::Vector2f(upper_left_anchor_position.x + m_panel->m_shape_container.getSize().x * 0.5f, upper_left_anchor_position.y + m_panel->m_shape_container.getSize().y * 0.5f));
